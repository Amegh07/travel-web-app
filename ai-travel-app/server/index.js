...
import { 
  ...
  Music, Utensils, Moon, Camera, Heart, Link as LinkIcon
} from 'lucide-react';
        ...
        >
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 text-xs text-slate-400">
                            <span className="bg-white/10 px-2 py-0.5 rounded text-slate-300">{isRoundTrip ? 'Round Trip' : 'One Way'}</span>
                ...
                <div className="text-right">
                    ...
                    {showBook && (
                        <button 
                            ...
                            className="mt-2 text-xs bg-emerald-500 hover:bg-emerald-400 text-black font-bold px-3 py-1.5 rounded-full flex items-center gap-1 ml-auto transition-colors"
                        >
                            <Globe size={12}/> Book
                        </button>
                    )}
            ...
            <div className="space-y-3">
                <div className="flex items-center gap-3 text-sm">
                    <Plane size={14} className="text-slate-500 rotate-45"/>
                    <div className="flex-1 flex justify-between items-center bg-black/20 p-2 rounded-lg">
                        <span className="font-mono text-white">{outbound?.segments[0]?.departure?.iataCode}</span>
                        <div className="flex flex-col items-center w-24">
                            <div className="h-[1px] w-full bg-slate-600"></div>
                            <span className="text-[10px] text-slate-500">{formatDuration(outbound?.duration)}</span>
                        </div>
                        <span className="font-mono text-white">{outbound?.segments[outbound.segments.length - 1]?.arrival?.iataCode}</span>
                    </div>
                </div>
                {isRoundTrip && (
                    <div className="flex items-center gap-3 text-sm">
                        <Plane size={14} className="text-slate-500 -rotate-135"/>
                        <div className="flex-1 flex justify-between items-center bg-black/20 p-2 rounded-lg">
                            <span className="font-mono text-white">{returnLeg?.segments[0]?.departure?.iataCode}</span>
                            <div className="flex flex-col items-center w-24">
                                <div className="h-[1px] w-full bg-slate-600"></div>
                                <span className="text-[10px] text-slate-500">{formatDuration(returnLeg?.duration)}</span>
                            </div>
                            <span className="font-mono text-white">{returnLeg?.segments[returnLeg.segments.length - 1]?.arrival?.iataCode}</span>
                        </div>
                    </div>
                )}
            </div>
...
const HotelCard = ({ hotel, isSelected, onSelect, nights, showBook = false }) => {
    ...
    return (
        <div 
            ...
            className={`
                group relative rounded-2xl overflow-hidden border transition-all cursor-pointer flex flex-col
                ${isSelected 
                    ? 'border-emerald-500 ring-2 ring-emerald-500/20 shadow-[0_0_20px_rgba(16,185,129,0.2)]' 
        ...
        >
            <div className="relative h-48 overflow-hidden bg-slate-800">
                ...
                <div className="absolute bottom-3 left-3 flex gap-2">
                    <span className="text-xs font-bold bg-black/50 backdrop-blur-md text-white px-2 py-1 rounded flex items-center gap-1">
                        <Star size={10} className="text-yellow-400 fill-yellow-400"/> {hotel.rating || "4.5"}
                    </span>
                </div>
            </div>
            ...
            <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="mt-auto border-t border-white/10 pt-3 flex justify-between items-end">
                    <div>
                        <div className="text-xs text-slate-500 font-mono mb-0.5">{nights} Nights • ${rawPrice}/night</div>
                        <div className="text-xl font-bold text-emerald-400">${totalPrice} <span className="text-xs text-slate-500 font-normal">Total</span></div>
                    </div>
                    {showBook && (
                        <button 
                            onClick={(e) => { e.stopPropagation(); window.open(`https://www.google.com/search?q=book+${hotel.name}`, '_blank'); }} 
                            className="text-xs bg-white text-black font-bold px-3 py-1.5 rounded-lg hover:scale-105 transition-transform"
                        >
                            Book
                        </button>
                    )}
...
// --- COMPONENT: EVENT CARD (TICKETMASTER) ---
    ...
    return (
        <div className={`relative flex gap-4 p-4 rounded-2xl border transition-all ${isAdded ? 'bg-pink-500/10 border-pink-500' : 'bg-white/5 border-white/5 hover:bg-white/10'}`}>
            ...
            <div className="flex-1 flex flex-col">
                ...
                <h4 className="text-white font-bold text-base leading-tight mb-2 line-clamp-1">{event.title}</h4>
                ...
                <div className="mt-auto pt-2 flex items-center justify-between">
                    <div className="flex items-center gap-3 text-xs text-slate-500">
                        ...
                        <span>• {event.price || "See Details"}</span>
                    </div>
                    ...
                    {event.url && (
                        <button 
                            ...
                            className="flex items-center gap-1 text-xs bg-pink-600 hover:bg-pink-500 text-white px-3 py-1.5 rounded-lg font-bold transition-colors"
                        >
        ...
        <div className="absolute inset-0 opacity-50 transition-opacity group-hover:opacity-70">
            ...
            {data && !loading && (
                <svg className="absolute inset-0 w-full h-full">
                    <motion.path initial={{ pathLength: 0 }} animate={{ pathLength: 1 }} transition={{ duration: 2, ease: "easeInOut" }} d="M 80 80 Q 200 200 320 250" stroke="url(#gradient-line)" strokeWidth="4" fill="none" strokeLinecap="round" />
      ...
      try {
        ...
        if (UNSPLASH_KEY) {
            ...
            if (Array.isArray(eventData) && eventData.length > 0) {
                 // For AI events only (Ticketmaster provides its own images)
